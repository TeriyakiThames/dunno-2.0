import { pgSchema, pgEnum, pgTable, integer, uuid, text, bigint, varchar, timestamp, bigserial, real, json, jsonb, numeric, date, customType, boolean, inet, smallint, index, uniqueIndex, foreignKey, primaryKey, unique, check, pgPolicy, doublePrecision } from "drizzle-orm/pg-core"
...
export const goal = pgEnum("goal", ["balanced", "moderate", "highProtein", "ketogenic"])
...
export const component = pgTable.withRLS("component", {
	...
	hasSeafood: boolean("has_seafood").default(false).notNull(),
	hasLactose: boolean("has_lactose").default(false).notNull(),
	hasPeanut: boolean("has_peanut").default(false).notNull(),
	hasGluten: boolean("has_gluten").default(false).notNull(),
});

export const dietProfile = pgTable.withRLS("diet_profile", {
	userId: uuid("user_id").primaryKey().references(() => user.id, { onDelete: "cascade" } ),
	lastUpdated: timestamp("last_updated", { precision: 3, withTimezone: true }).default(sql`now()`).notNull(),
	calorieIntake: real("calorie_intake").default(0).notNull(),
	carbsIntake: real("carbs_intake").default(0).notNull(),
	proteinIntake: real("protein_intake").default(0).notNull(),
	fatIntake: real("fat_intake").default(0).notNull(),
	alcoholIntake: real("alcohol_intake").default(0).notNull(),
	streak: integer().default(0).notNull(),
}, (table) => [

	pgPolicy("Allow user to do anything with their diet profile", { to: ["authenticated"], using: sql`(( SELECT auth.uid() AS uid) = user_id)`, withCheck: sql`(( SELECT auth.uid() AS uid) = user_id)` }),
]);

export const dish = pgTable.withRLS("dish", {
	id: integer().primaryKey().generatedAlwaysAsIdentity(),
	nameTh: text("name_th").notNull(),
	nameEn: text("name_en").notNull(),
	resId: integer("res_id").notNull(),
	price: real().notNull(),
});

export const dishComponentMap = pgTable.withRLS("dish_component_map", {
	dishId: integer("dish_id").notNull().references(() => dish.id, { onDelete: "cascade" } ),
	componentId: integer("component_id").notNull().references(() => component.id, { onDelete: "cascade" } ),
	ratio: real().default(1).notNull(),
});

export const mealHistory = pgTable.withRLS("meal_history", {
	id: integer().primaryKey().generatedAlwaysAsIdentity(),
	userId: uuid("user_id").references(() => user.id, { onDelete: "cascade" } ),
	dishId: integer("dish_id").references(() => dish.id, { onDelete: "cascade" } ),
	at: timestamp({ precision: 3, withTimezone: true }).default(sql`now()`).notNull(),
	editedCarbs: real("edited_carbs").default(0).notNull(),
	editedProtein: real("edited_protein").default(0).notNull(),
	editedFat: real("edited_fat").default(0).notNull(),
	editedAlcohol: real("edited_alcohol").default(0).notNull(),
});

export const restaurant = pgTable.withRLS("restaurant", {
	id: integer().primaryKey().generatedAlwaysAsIdentity(),
	nameTh: text("name_th").notNull(),
	nameEn: text("name_en").notNull(),
	lat: numeric({ precision: 9, scale: 7 }),
	lon: numeric({ precision: 10, scale: 7 }),
	url: text().notNull(),
	hasDineIn: boolean("has_dine_in").notNull(),
	hasDelivery: boolean("has_delivery").notNull(),
});

export const restaurantType = pgTable.withRLS("restaurant_type", {
	id: integer().primaryKey().generatedAlwaysAsIdentity(),
	type: text().notNull(),
});

export const restaurantTypeMap = pgTable.withRLS("restaurant_type_map", {
	resId: integer("res_id").notNull().references(() => restaurant.id, { onDelete: "cascade" } ),
	typeId: integer("type_id").notNull().references(() => restaurantType.id, { onDelete: "cascade" } ),
});
...
export const user = pgTable.withRLS("user", {
	...
	createdAt: timestamp("created_at", { precision: 3, withTimezone: true }).default(sql`now()`).notNull(),
	vegetarianDefault: boolean("vegetarian_default").default(false).notNull(),
	halalDefault: boolean("halal_default").default(false).notNull(),
	noSeafoodDefault: boolean("no_seafood_default").default(false).notNull(),
	noLactoseDefault: boolean("no_lactose_default").default(false).notNull(),
	noPeanutDefault: boolean("no_peanut_default").default(false).notNull(),
	glutenFreeDefault: boolean("gluten_free_default").default(false).notNull(),
	goal: goal().default("balanced").notNull(),
	targetCalorie: real("target_calorie"),
	targetFat: real("target_fat"),
	targetProtein: real("target_protein"),
	targetCarbs: real("target_carbs"),
}, (table) => [
	unique("user_email_unique").on(table.email),
	pgPolicy("Allow user to do anything with their row", { to: ["authenticated"], using: sql`(( SELECT auth.uid() AS uid) = id)`, withCheck: sql`(( SELECT auth.uid() AS uid) = id)` }),
check("activity_level check", sql`(activity_level = ANY (ARRAY[(1.2)::real, (1.375)::real, (1.55)::real, (1.725)::real, (1.9)::real]))`),]);
